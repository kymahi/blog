read -p "Enter Episode year (yyyy): " year
read -p "Enter Episode month (mm): " month
read -p "Enter Episode day (dd): " day
...
oldRankings=$(ls -1 _posts | grep -Ei "week-\d+.md" | tail -n 1)

newOutline="$year-$month-$day-episode-$epNum"-outline.md
...
newRankings="$year-$month-$day-power-rankings-week-$rankingsWeek".md
